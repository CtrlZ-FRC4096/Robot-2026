...
from pathplannerlib.path import PathConstraints, GoalEndState, PathPlannerPath, Waypoint
import math
...
class ADStar():
    ...
    _fieldLength: float = FieldConstants.fieldLength
    _fieldWidth: float = FieldConstants.fieldWidth
    ...
    def _createWaypoints(self, path: List[GridPosition], real_start_pos: Translation2d,
        ...
        for i in range(1, len(fieldPosPath) - 1):
            ...
            anchor1 = ((current - last) * ADStar._SMOOTHING_ANCHOR_PCT) + last
            heading1 = (current - last).angle()
            anchor2 = ((current - next) * ADStar._SMOOTHING_ANCHOR_PCT) + next
