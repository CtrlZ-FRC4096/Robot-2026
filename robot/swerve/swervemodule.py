...
class SwerveModule:
    ...
    def __init__(
        ...
        swerve_drive_motor_config.current_limits.supply_current_limit = (
            100  # I am not sure if this is correct
        )
        swerve_drive_motor_config.current_limits.supply_current_limit = (
            100  # change back to 40
        )

        swerve_drive_motor_config.torque_current.peak_forward_torque_current = (
            100  # Up this to 80 for more zip
        )
        swerve_drive_motor_config.torque_current.peak_reverse_torque_current = -100
        ##Ramps
